...
class User_model extends CI_Model {
	...
	public function get_active_user(){
        ...
        public function check_username_exists($email){
            ...
            $rs=$this->db->from($this->_table)->where('userName',$email)->where('status <','2')->get()->result();
            if(count($rs)>0){
                return TRUE;
            }else{
                return FALSE;
	...
	public function check_login_data($email,$Password){
		...
		$this->db->select('*')->from($this->_table)->where('userName',$email)->where('Password',base64_encode($Password))->where('status','1')->where('userType','0');
